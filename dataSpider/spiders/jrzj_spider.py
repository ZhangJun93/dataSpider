...
class JRZJSpider(BaseSpider):
    name = "jrzj_spider"
    website = u"金融之家"
    allowed_domains = ['jrzj.com']
    start_urls = ['https://www.jrzj.com/']

    rules = [
        Rule(LinkExtractor(allow=(r'.*html')), callback='parse_item'),
        Rule(LinkExtractor(allow=('/column/', '/news', '/zixun/', '/event', '/report/')), callback=None, follow=True),
    ]
    ...
    def parse_item(self, response):
        ...
        content = ''
        try:
            title = response.xpath(r'//*[@class="infor_border"]/h1//text()').extract()
            date_raw = response.xpath(r'//*[@class="right_sc"]//text()').extract()
