...
class CXJRSpider(BaseSpider):
    name = "cxjr_spider"
    website = u"财新金融"
    allowed_domains = ['caixin.com']
    start_urls = ['http://finance.caixin.com/']

    rules = [
        Rule(LinkExtractor(allow=('/regulation/', '/bank/', '/stock/', '/insurance_trust/', '/investment/', '/innovation/', '/market/', '/assets/')), follow=True),
        Rule(LinkExtractor(allow=(r'finance.*html')), callback='parse_item', follow=True),
    ]
    ...
    def parse_item(self, response):
        ...
        content = ''
        try:
            title = response.xpath(r'//*[@class="article"]/div[1]/h1//text()').extract()
            date = response.xpath(r'//*[@class="artInfo"]//text()').extract_first()
            if date is not None:
